...
@property (weak  , nonatomic) UIView                     *weekContainer;
...
- (void)initialize
{
    ...
    UIView *weekContainer = [[UIView alloc] initWithFrame:CGRectZero];
    // weekContainer.backgroundColor = [UIColor colorWithRed:230.0/255.0 green:76.0/255.0 blue:60.0/255.0 alpha:1];
   // weekContainer.backgroundColor = [UIColor purpleColor];
    [contentView addSubview:weekContainer];
    self.weekContainer = weekContainer;
...
- (void)layoutSubviews
{
    ...
    if (_needsAdjustingViewFrame) {
        ...
        if (_scopeHandle) {
            ...
            _weekContainer.frame = CGRectMake(0, 46, self.fs_width, 20);
            _weekContainer.clipsToBounds=YES;
            _weekContainer.layer.cornerRadius=10;
        }
        ...
        if (!self.floatingMode) {
            switch (_scope) {
                case FSCalendarScopeMonth: {
                    ...
                    _daysContainer.frame = CGRectMake(0, headerHeight+weekdayHeight+10, self.fs_width, contentHeight);
    ...
    if (!self.floatingMode) {
        ...
        if (!_weekdays.count) {
            ...
            for (int i = 0; i < weekSymbols.count; i++) {
                ...
                weekdayLabel.text = [[weekSymbols[i] substringToIndex:3] uppercaseString];
